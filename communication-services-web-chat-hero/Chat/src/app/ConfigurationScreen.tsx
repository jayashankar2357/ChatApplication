...
export default (props: ConfigurationScreenProps): JSX.Element => {
  ...
  const setupAndJoinChatThreadWithNewUser = useCallback(() => {
    const internalSetupAndJoinChatThread = async (): Promise<void> => {
      ...
      if (!threadId) {
        throw new Error(ERROR_TEXT_THREAD_NULL+emptyWarning);
      }
      ...
      const result = await joinThread(threadId, token.identity, setDisplayName);
      if (!result) {
  ...
  }, [setDisplayName, joinChatHandler, selectedAvatar,  setEndpointUrl, setThreadId, setToken, setUserId]);
    ...
    return (
      ...
      >
        <Stack horizontalAlign="center" tokens={leftPreviewContainerStackTokens} className={leftPreviewContainerStyle}>
          ...
          <Text className={namePreviewStyle(name !== '')}>{setDisplayName !== '' ? setDisplayName : NAME_DEFAULT}</Text>
        </Stack>
